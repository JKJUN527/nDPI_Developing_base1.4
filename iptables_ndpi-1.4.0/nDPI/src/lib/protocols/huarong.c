 ...
 /*
  ...
  if(packet->payload_packet_len >= (16)
  	...
  	&&((packet ->payload[0]==0x0c
		...
		&&(packet->payload[3]==0x01||packet->payload[3]==0x00)))
  ){			
  		NDPI_LOG(NDPI_PROTOCOL_HUARONG, ndpi_struct, NDPI_LOG_DEBUG,"found huarong------tcp[0[0c|b1]:%x]tcp[2[18|74]:%x]tcp[4[00|0c]:%x] \n",packet->payload[0],packet->payload[2],packet->payload[4]);
