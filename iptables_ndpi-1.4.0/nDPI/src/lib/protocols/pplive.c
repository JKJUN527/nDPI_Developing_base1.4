				...
				*ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  if (packet->udp != NULL) {
	...
	&& (packet->payload[17] == 0) && (packet->payload[24] == 0xac)) {
      NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive 1.\n");
...
/*jkjun修改，特征太小，与战舰世界登录流量冲突
    if ((packet->payload[7] == 0x00&& packet->payload[8] == 0x00&& packet->payload[9] == 0x0c &&packet->payload[10] == 0x01) || (packet->payload[7] == 0x00 &&packet->payload[8] == 0x00)) {
*/
if ((packet->payload[7] == 0x00||packet->payload[7] == 0x01)
     && packet->payload[8] == 0x00
     &&((packet->payload[9] == 0x0c &&packet->payload[10] == 0x01)
	||(packet->payload[9] == 0x01 &&packet->payload[10] == 0x0c))
) {

      NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive 2.\n");
      ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
	...
	&& packet->payload[6] == 0x01 && packet->payload[7] == 0x02) {
      NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive 3.\n");
      ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
	...
	&& ((get_u_int32_t(packet->payload, 58) == ntohl(0xb1130000))
		...
		&& (packet->payload[78] == 0x00 || packet->payload[78] == 0x01)
		&& (packet->payload[79] == 0x00 || packet->payload[79] == 0x01)))) {
      NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive 4.\n");
      ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
	...
	if(packet->payload_packet_len ==94  
		&& packet->payload[8] == 0x00
		//&&get_u_int16_t(packet->payload, 22)==ntohl(0xc859)
		//&&(get_u_int32_t(packet->payload, 24)==ntohl(0x90f5ea53)
		&&packet->payload[4]==0x52
		&&packet->payload[7]==0x00
	){
	 NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive 5.\n");
      	 ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
         return;
	}
	...
	dst->pplive_last_packet_time_set = 1;
      ...
      NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive 6.\n");
      ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
	...
	NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct, NDPI_LOG_DEBUG, "found pplive 7.\n");
	ndpi_int_pplive_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	return;
      }
