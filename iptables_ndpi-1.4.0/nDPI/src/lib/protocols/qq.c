...
#define DEBUG_QQ 1
//#ifdef DEBUG_QQ
//NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ \n");
//#endif
...
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_QQ, NDPI_CORRELATED_PROTOCOL);
}
...
u_int8_t ndpi_is_valid_qq_packet(const struct ndpi_packet_struct *packet)
{
 ...
 /* 17,81,   */
  ...
  case 0x0108:	/* May file transport or voice */
  case 0x0065:	/* Request extra information */
  case 0x003c:	/* Group name opration */
  case 0x03c9:	/* May voice communicate*/
  case 0x0125:	/*  */
  case 0x01e0:	/*  */
  case 0x01c0:	/*  */
  case 0x01c8:	/*  */
...
static void ndpi_search_qq_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  ...
  static const u_int16_t p8000_patt_02[16] =	// maybe version numbers
    { 0x1549, 0x1801, 0x180d, 0x0961, 0x01501, 0x0e35, 0x113f, 0x0b37, 0x1131, 0x163a, 0x1e0d, 0x3639, 0X3649, 0x3703, 0x360d/*QQ7.2*/, 0x3661 /*when forbit many,appear a new version id*/ };
  u_int16_t no_of_patterns = 16, index = 0;
//goto qq_pt_check;
  ...
  if (flow->qq_stage <= 3) {
	    #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #1 stage:%d\n",flow->qq_stage);
  #endif
	...
	return;
      ...
      flow->qq_stage++;
        #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #2 stage:%d\n",flow->qq_stage);
  #endif
      if (flow->qq_stage == 3) {
	...
	if (/*pat == p8000_patt_02[index] &&*/ packet->payload[packet->payload_packet_len - 1] == 0x03) {
	  ...
	  #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #3 stage:%d\n",flow->qq_stage);
  #endif
	  return;
	}
      }
	...
	if (pat == p8000_patt_02[index]) {
	  flow->qq_stage++;
	  #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #4 stage:%d\n",flow->qq_stage);
  #endif
	  /*
	  ...
	  #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #5 stage:%d\n",flow->qq_stage);
  #endif
	  return;
	}
      }
	...
	&& packet->payload[packet->payload_packet_len - 1] == 0x03) {
      flow->qq_stage++;
      #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #6 stage:%d\n",flow->qq_stage);
  #endif
      if (flow->qq_stage == 3) {
	NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG,
	...
	&& (packet->payload[packet->payload_packet_len - 1] == 0x00
      ...
      flow->qq_stage++;
      #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #7 stage:%d\n",flow->qq_stage);
  #endif
      if (flow->qq_stage == 3) {
	NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG,
	...
	&& ntohs(get_u_int16_t(packet->payload, 1)) == 0x1131 && packet->payload[packet->payload_packet_len - 1] == 0x03) {
      flow->qq_stage++;
      #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #8 stage:%d\n",flow->qq_stage);
  #endif
      if (flow->qq_stage == 3) {
	NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG,
	...
	get_u_int16_t(packet->payload, 4) == htons(0x0b0b)) {
      flow->qq_stage++;
      #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #9 stage:%d\n",flow->qq_stage);
  #endif
      if (flow->qq_stage == 3) {
	NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG,
	...
	flow->qq_stage++;
	#ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #10 stage:%d\n",flow->qq_stage);
  #endif
	if (flow->qq_stage == 3) {
	  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG,
...
//qq_pt_check:
  if (ndpi_is_valid_qq_packet(packet)) {
    flow->qq_stage++;
    #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #11 stage:%d\n",flow->qq_stage);
  #endif
    if (flow->qq_stage > 0) {  /* default  stage == 3*/
  ...
  if (ndpi_is_valid_qq_ft_packet(packet)) {
    flow->qq_stage++;
    #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #12 stage:%d\n",flow->qq_stage);
  #endif
    if (flow->qq_stage == 3) {
  ...
  if (flow->qq_stage && flow->packet_counter <= 5) {
	  #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #13 stage:%d\n",flow->qq_stage);
  #endif
    return;
  }
...
#ifdef NDPI_PROTOCOL_QQ2

static void ndpi_int_qq2_add_connection(struct ndpi_detection_module_struct *ndpi_struct,
				       struct ndpi_flow_struct *flow,
				       ndpi_protocol_type_t protocol_type)
{
  ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_QQ2, protocol_type);
}
...
static void ndpi_search_qq2_udp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  struct ndpi_packet_struct *packet = &flow->packet;
	
  static const u_int16_t p8000_patt_02[13] =	// maybe version numbers
    { 0x1549, 0x1801, 0x180d, 0x0961, 0x01501, 0x0e35, 0x113f, 0x0b37, 0x1131, 0x163a, 0x1e0d, 0x3639 };
  u_int16_t no_of_patterns = 12, index = 0;
//goto qq_pt_check;

  NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "search qq udp.\n");


  if (flow->qq_stage <= 3) {
  #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #1\n");
  #endif
	...
	|| (packet->payload_packet_len > 10
		...
		|| (get_u_int32_t(packet->payload, 0) == htonl(0x04163a00)
		    ...
		    && packet->payload[4] == packet->payload_packet_len)))) {
      /*
	if (flow->qq_stage == 3 && flow->detected_protocol == NDPI_PROTOCOL_QQ2) {
	if (flow->packet_direction_counter[0] > 0 && flow->packet_direction_counter[1] > 0) {
	flow->protocol_subtype = NDPI_PROTOCOL_QQ2_SUBTYPE_AUDIO;
	return;
	} else if (flow->packet_counter < 10) {
	return;
	}
	  ...
	  #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #2\n");
  #endif
      flow->qq_stage++;
      if (flow->qq_stage == 3) {
	NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG,
		 "found qq udp pattern 030001 or 000e35 four times.\n");
	ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	return;
      }
        #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #3\n");
  #endif
      return;
    }
      #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #4\n");
  #endif
    if (packet->payload_packet_len > 2 && (packet->payload[0] == 0x02 || packet->payload[0] == 0x04)) {
      u_int16_t pat = ntohs(get_u_int16_t(packet->payload, 1));
      for (index = 0; index < no_of_patterns; index++) {
  ...
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #5 stage:%d\n",flow->qq_stage);
      ...
      #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #6\n");
  #endif
    if (packet->payload_packet_len == 84 && (packet->payload[0] == 0 || packet->payload[0] == 0x03)) {
      u_int16_t pat = ntohs(get_u_int16_t(packet->payload, 1));
      for (index = 0; index < no_of_patterns; index++) {
  ...
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #7 stage:%d\n",flow->qq_stage);
	  ...
	  /*
	    if (flow->qq_stage == 3 && flow->packet_direction_counter[0] > 0 &&
	    flow->packet_direction_counter[1] > 0) {
	    NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern four times.\n");
	    ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	    return;
	    } else */ if (flow->qq_stage == 3) {
	    NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern four times.\n");
...
//qq_pt_check:
  ...
  #endif
    if (flow->qq_stage > 0) {  /* default  stage == 3*/
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq over udp.\n");
qq_ok:      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
    NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq packet stage %d\n", flow->qq_stage);
    return;
  }
  ...
  #endif
    if (flow->qq_stage == 3) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq ft over udp.\n");
      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
    return;
  ...
  if (flow->qq_stage && flow->packet_counter <= 5) {
  #ifdef DEBUG_QQ
  NDPI_LOG(NDPI_PROTOCOL_QQ, ndpi_struct, NDPI_LOG_DEBUG, "DEBUG:QQ #14 stage:%d\n",flow->qq_stage);
  #endif	  
    return;
  }

  NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "QQ excluded\n");
  NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_QQ2);
}
...
__forceinline static
#endif
void ndpi_search_qq2_tcp(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  struct ndpi_packet_struct *packet = &flow->packet;
  ...
  NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "search qq tcp.\n");

  if (packet->payload_packet_len == 39 && get_u_int32_t(packet->payload, 0) == htonl(0x27000000) &&
      get_u_int16_t(packet->payload, 4) == htons(0x0014) && get_u_int32_t(packet->payload, 11) != 0 &&
      get_u_int16_t(packet->payload, packet->payload_packet_len - 2) == htons(0x0000)) {
    if (flow->qq_stage == 4) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp - maybe ft/audio/video.\n");
      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
    flow->qq_stage = 4;
  ...
  if ((packet->payload_packet_len > 4 && ntohs(get_u_int16_t(packet->payload, 0)) == packet->payload_packet_len
    ...
    flow->qq_stage++;
    if (flow->qq_stage == 3) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp.\n");
      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
    return;
  ...
  if (ndpi_is_valid_qq_ft_packet(packet)) {
    flow->qq_stage++;
    if (flow->qq_stage == 3) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq ft over tcp.\n");
      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
    return;
  ...
  if (packet->payload_packet_len > 5 && (((flow->l4.tcp.qq_nxt_len == packet->payload_packet_len + 2)
    ...
    flow->qq_stage++;
    if (flow->qq_stage == 3) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq udp pattern 02 ... 03 four times.\n");
      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
      return;
    }
    return;
  ...
  if (packet->payload_packet_len > 100
      && ((ndpi_mem_cmp(packet->payload, "GET", 3) == 0) || (ndpi_mem_cmp(packet->payload, "POST", 4) == 0))) {
    NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found GET or POST.\n");
    if (memcmp(packet->payload, "GET /qqfile/qq", 14) == 0) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp GET /qqfile/qq.\n");
      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
      return;
    }
    ndpi_parse_packet_line_info(ndpi_struct, flow);
    ...
    if (packet->user_agent_line.ptr != NULL
	&& (packet->user_agent_line.len > 7 && memcmp(packet->user_agent_line.ptr, "QQClient", 8) == 0)) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp GET...QQClient\n");
      ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
      return;
    }
    for (i = 0; i < packet->parsed_lines; i++) {
      if (packet->line[i].len > 3 && memcmp(packet->line[i].ptr, "QQ: ", 4) == 0) {
	NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp GET...QQ: \n");
	ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
	return;
      }
    }
    if (packet->host_line.ptr != NULL) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "host line ptr\n");
      if (packet->host_line.len > 11 && memcmp(&packet->host_line.ptr[0], "www.qq.co.za", 12) == 0) {
	NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq over tcp Host: www.qq.co.za\n");
	ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_CORRELATED_PROTOCOL);
	return;
      }
    }
  ...
  if (flow->qq_stage == 0 && packet->payload_packet_len == 82
	...
	break;
      ...
      if (i == 81) {
	NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq Mail.\n");
	ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	return;
      }
    }
  ...
  if (packet->payload_packet_len == 204 && flow->qq_stage == 0 && get_u_int32_t(packet->payload, 200) == htonl(0xfbffffff)) {
    for (i = 0; i < 200; i++) {
      NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "i = %u\n", i);
      if (packet->payload[i] != 0) {
	break;
      }
      if (i == 199) {
	NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "found qq chat or file transfer\n");
	ndpi_int_qq2_add_connection(ndpi_struct, flow, NDPI_REAL_PROTOCOL);
	return;
      }
    }
  ...
  if (NDPI_COMPARE_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_HTTP) != 0) {
    ...
    NDPI_ADD_PROTOCOL_TO_BITMASK(flow->excluded_protocol_bitmask, NDPI_PROTOCOL_QQ2);
    NDPI_LOG(NDPI_PROTOCOL_QQ2, ndpi_struct, NDPI_LOG_DEBUG, "QQ tcp excluded; len %u\n",
	     packet->payload_packet_len);
...
void ndpi_search_qq2(struct ndpi_detection_module_struct *ndpi_struct, struct ndpi_flow_struct *flow)
{
  struct ndpi_packet_struct *packet = &flow->packet;
	
  if (packet->udp != NULL && flow->detected_protocol_stack[0] != NDPI_PROTOCOL_QQ2)
    ndpi_search_qq2_udp(ndpi_struct, flow);

  if (packet->tcp != NULL && flow->detected_protocol_stack[0] != NDPI_PROTOCOL_QQ2)
    ndpi_search_qq2_tcp(ndpi_struct, flow);
}
