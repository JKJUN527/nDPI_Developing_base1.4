...
static void setupFlowmeter(void);
// cli options
...
/*real time flag*/
// static int32_t want_5s_flag = 5; 
static u_int8_t daemon_flag = 0;
pthread_t tid5s;
static u_int8_t keep_data_rt = 0;
...
static void help(u_int long_help) {
  printf("flowmeter -c <config> -i <file|device> [-f <filter>][-s <duration>]\n"
	 "           [-p <protos>][-l <loops>[-d][-h][-t][-v <level>]\n\n"
	 "Usage:\n"
	 "  -c <config>               | Specify a config, validkey is URL_CONFIG_FP,URL_POSTFIX, IP_FILE\n"
	 "  -i <file.pcap|device>     | Specify a pcap file to read packets from or a device for live capture\n"
	 "  -f <BPF filter>           | Specify a BPF filter for filtering selected traffic\n"
//	 "  -s <duration>             | Maximum capture duration in seconds (live traffic capture only)\n"
	 "  -K <int times of 5s count>| count of 5s to walk data to disk\n"
...
static calcFlowFinal(){
	...
	if (shutdown_app && ! called){	
		called = 1;	
		calcFlowThread(hashtable, app_struct, 1, 1, keep_data_rt);
	}
}
...
static void parseOptions(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "c:Ddf:i:hkK:p:l:s:tv:")) != EOF) {
    switch (opt) {
    case 'c':
...
void setupFlowmeter(void){
	char t;
	printf("setupFlowmeter start\n");
	read_config(config_file_path, GLOBAL_CONF);
	set_config_default(GLOBAL_CONF);
	if (daemon_flag)
		daemon_fm();
	// handle ip by file
	all_ip = load_all_ip_file(GLOBAL_CONF[IP_FILE].value, &all_ip_cnt);
	// printf("will quick sort\n");
   	// dumpIPS(all_ip, all_ip_cnt); 
	// quick_sort(all_ip, 0, all_ip_cnt-1);
   	// dumpIPS(all_ip, all_ip_cnt); 
	/*INIT structs*/
	if((t = initFlowmeterStruct(&hashtable, &app_struct)) != 0){
		printf("initFlowmeterStruct fail code:%d\n",t);
		exit(-1);
	}	
	initLogHandler();
	printf("setFlowmeter OK\n");
}
				      ...
				      u_int16_t ipsize, u_int16_t rawsize)
{
  ...
  if(flow->detection_completed){
	// printf("detection_completed\n");
	updateFlowData(ndpi_struct, flow, hashtable, app_struct);
	return(0);
  }


  #ifdef DEBUG
  printf("[zllz] ----- before ndpi_detection_process ndpi_flow:{%s}\n",\
  ndpi_flow->packet.payload);
  ...
  #if 0
    printf("[NDPI][NDPI2] -----after process_packet. top payload:%s \n",ndpi_flow->packet.payload);//no any payload here
  #endif
...
void sigproc5s(int sig){
  static int called = 0;
  printf("get 5s data signal!\n");
  want_5s_flag = 5;
}
...
void test_lib() {
  ...
  pthread_create(&tid5s, NULL, fiveSecThread, NULL);
...
void printArgs()
{
      ...
      printf("\tdata will output every %u sec\n", COUNT_TIMES*5);
}
