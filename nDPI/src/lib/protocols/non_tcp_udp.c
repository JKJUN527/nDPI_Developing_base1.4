...
void ndpi_search_ipsec(struct ndpi_detection_module_struct
				*ndpi_struct, struct ndpi_flow_struct *flow)
{
  struct ndpi_packet_struct *packet = &flow->packet;
  if(packet->payload_packet_len >= 16){
	if((packet->payload[0]==0x50
     	    &&packet->payload[1]==0x49
     	    &&packet->payload[2]==0x0a
     	    &&packet->payload[3]==0xf9)
	||(packet->payload[4]==0x50
     	    &&packet->payload[5]==0x49
     	    &&packet->payload[6]==0x0a
     	    &&packet->payload[7]==0xf9)
	||(packet->payload[4]==0xf3
     	    &&packet->payload[5]==0xaf
     	    &&packet->payload[6]==0xfd
     	    &&packet->payload[7]==0x43)
    //esp(SPI)Ê¶±ð
    ||(packet->payload[0]==0x27
     	    &&packet->payload[1]==0xcc
     	    &&packet->payload[2]==0x55
     	    &&packet->payload[3]==0x07)
    ||(packet->payload[0]==0x16
     	    &&packet->payload[1]==0xeb
     	    &&packet->payload[2]==0xfb
     	    &&packet->payload[3]==0x86)

	){
  		NDPI_LOG(NDPI_PROTOCOL_IPSEC, ndpi_struct, NDPI_LOG_DEBUG,"found ipsec \n");
		NDPI_LOG(NDPI_PROTOCOL_IPSEC, ndpi_struct, NDPI_LOG_DEBUG,"sport:%u,%d,dport:%u,%d \n",packet->udp->source,packet->udp->source,packet->udp->dest,packet->udp->dest);
...
NDPI_LOG(NDPI_PROTOCOL_IPSEC, ndpi_struct, NDPI_LOG_DEBUG,"search non_tcp_udp \n");
NDPI_LOG(NDPI_PROTOCOL_IPSEC, ndpi_struct, NDPI_LOG_DEBUG,"protocol:%u \n",packet->l4_protocol);
