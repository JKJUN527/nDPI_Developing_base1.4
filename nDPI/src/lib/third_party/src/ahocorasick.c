...
AC_AUTOMATA_t * ac_automata_init (MATCH_CALBACK_f mc)
{
  ...
  #ifdef AC_DEBUG
	printf("ac_automata_init size:%lu \n", thiz->all_nodes_max*sizeof(AC_NODE_t *));
  #endif
  
  thiz->all_nodes = (AC_NODE_t **) ndpi_malloc (thiz->all_nodes_max*sizeof(AC_NODE_t *));
  #ifdef AC_DEBUG
  if(thiz->all_nodes == NULL){
    printf("malloc fail for thiz->all_nodes #1\n");
  }else{
	 printf("malloc success for thiz->all_nodes #1\n");
  }
...
#ifdef AC_DEBUG
	if(thiz->all_nodes == NULL){
	  printf("malloc fail for thiz->all_nodes #2\n");
	}else{
	   printf("malloc success for thiz->all_nodes #2\n");
	}
  ...
  for (i=0; i<patt->length; i++)
      ...
      else
	{
	  ...
	  ac_automata_register_nodeptr(thiz, n);
	    #ifdef AC_DEBUG
		if(thiz->all_nodes == NULL){
	  		printf("malloc fail for thiz->all_nodes #3\n");
		}else{
	   		printf("malloc success for thiz->all_nodes #3\n");
		}
		#endif
	}
    }
...
void ac_automata_finalize (AC_AUTOMATA_t * thiz)
{
  ...
  if((alphas = ndpi_malloc(AC_PATTRN_MAX_LENGTH)) != NULL) {
  	#ifdef AC_DEBUG
	printf("ac_automata_traverse_setfailure start (depth:%d)\n", thiz->root->depth);
	#endif
    ac_automata_traverse_setfailure (thiz, thiz->root, alphas);
	#ifdef AC_DEBUG
		printf("ac_automata_traverse_setfailure end\n");
	#endif
	...
	#ifdef AC_DEBUG
	printf("node_sort_edges %u of %u\n",i, thiz->all_nodes_num);
	#endif
	node_sort_edges (node);
	#ifdef AC_DEBUG
	printf("node_sort_edges ok\n");
	#endif
      }
...
(AC_AUTOMATA_t * thiz, AC_NODE_t * node, AC_ALPHABET_t * alphas)
{
  ...
  for (i=0; i < node->outgoing_degree; i++)
	...
	#ifdef AC_DEBUG
	  printf("ac_automata_set_failure (i:%u id %u  cnt:%lu node_depth:%d\n", i, node->id, cnt, node->depth);
	#endif
      /* At every node look for its failure node */
      ac_automata_set_failure (thiz, next, alphas);

	#ifdef AC_DEBUG
			printf("--IN recursive ac_automata_set_failure in (i:%u id:%u cnt:%lu)\n",i, node->id, cnt);
	#endif

      /* Recursively call itself to traverse all nodes */
      ac_automata_traverse_setfailure (thiz, next, alphas);
	#ifdef AC_DEBUG
			printf("--OUT recursive ac_automata_set_failure out (i:%u id:%u cnt:%lu)\n",i, node->id, cnt);
	#endif
    }
}
